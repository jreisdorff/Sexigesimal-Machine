#!/usr/bin/env python3
"""Expand glyph_script.txt (VCD-style shorthand) into tb_sex_glyph.vhdl stim (base-223 FSM)."""
...
def write_glyphs_reference(
    ...
    lines_out: list[str] = []
    lines_out.append("# Base-223 FSM VCD glyphs (GHDL tb_sex dump codes)")
    ...
    lines_out.append("&   tb_sex.digit_o[7:0]               no (DUT out)")
    ...
    lines_out.append("-   tb_sex.dut.digit_o[7:0]           no")
    lines_out.append(".   tb_sex.dut.wrap_o                 no")
    lines_out.append("/   tb_sex.dut.digit_r[7:0]           no")
    lines_out.append("0   tb_sex.dut.digit_x[7:0]           no (id is digit zero)")
...
architecture sim of tb_sex_glyph is
  ...
  signal digit_o : unsigned(7 downto 0);
